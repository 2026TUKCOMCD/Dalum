...
def run():
    ...
    with open(CSV_PATH, encoding="utf-8") as f:
        ...
        for i, row in enumerate(reader, 1):
            ...
            top3_materials, material_prob_dict = material_predictor.predict_from_array(
                enhanced
            )

            # postprocessor는 dict 필요
            ...
            # npy 저장용 vector
            if isinstance(material_prob_dict, dict):
                material_vector = np.array(
                    list(material_prob_dict.values()),
                    dtype=np.float32
                )
            else:
                material_vector = np.array(material_prob_dict, dtype=np.float32)

            print(f"{filename} → 재질: {material_label}")

            # 최종 임베딩 벡터 (color + material)
